...
`=`，`+`，`-`，`@`で始まる値は，先頭へ`'`を付けて表計算ソフトが数式として評価しないようにする．引用しただけでは数式として解釈されるため，引用とは別に必要である．共有リストは第三者から届くため，受け取った人がExcelで開いた時点で数式が動くことを防ぐ．数量と価格は0以上の整数であり，この処理の対象にならない．

#### 共通TSV

CSVと同じ列をタブ区切りで出力する．値にタブや改行が含まれる場合は空白へ正規化する．表計算ソフトへ貼り付けられるため，CSVと同じ数式の無害化を行う．
...
- 背景クリックで閉じる
- Escキーで閉じる．モーダルが重なっている場合は最前面だけを閉じる．編集画面の上へ出したエラーを閉じるつもりで，編集内容を失わせない
- 閉じるボタンを表示する
- 画面外へはみ出さない
- 商品数が多い場合は表部分だけスクロールする
- 開いている間は背後のページのスクロールを止める
- Tabキーによるフォーカスをモーダル内へ閉じ込め，閉じたら呼び出し元の要素へフォーカスを戻す．`aria-modal="true"`を宣言する以上，実装を伴わせる
- 主要操作前には確認メッセージを表示する
- 破壊的操作は目立つ位置へ置かない
- 確認と1行入力は`window.confirm`と`window.prompt`を使わず，Cart2BOMのモーダルで統一する．ブラウザ標準ダイアログは見た目が揃わず，抑制される場合もある
...
トーストが連続する場合は縦に積み上げ，前の通知を隠さない．

エラーはモーダル内またはダイアログで詳細を表示する．
読み取り警告がある場合は件数だけでなく、対象商品の手掛かりと理由を読み取り結果モーダル内へ表示する．

通販サイトの応答を待つ処理には，進行中であることを示す表示を出す．ミスミはカート読み取りで最大8秒，バスケットへの追加では各段階で最大20秒待つため，表示がないと操作が効いていないように見える．表示は処理の成否にかかわらず必ず閉じる．
...
読み取りは1件単位で切り分ける．保存領域の値が配列でない場合だけ全体を失敗させ，配列内の個々のリストが検証に失敗した場合は，そのリストだけを除外して残りを表示する．除外したリストは元の値のまま保持し，保存・更新・削除で書き戻すときも消さない．保存済みリスト画面には除外件数と，除外分をJSONへ書き出す操作を表示する．

これにより，`schemaVersion`を上げたときやデータが一部壊れたときに，利用者が他のリストを失わずに済む．
...
重複処理は，アダプターの読み取り直後，カート確認画面を開く前に適用する．統合した件数と，単価不一致で統合しなかった商品は，読み取り警告として確認画面へ表示する．
...
- スキーママイグレーションは未実装である．`cart2bom.migrations`キーとマイグレーション関数の型は定義しているが適用処理がなく，`schemaVersion`が現行値と一致しないデータは検証で拒否する．11.1の切り分けにより他のリストは読めるが，該当リストは利用できない．スキーマを変更する場合は，先に適用処理を実装する
...
- `consumePendingQuickOrder`はどこからも呼ばれていない
