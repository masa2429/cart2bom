...
モノタロウは一括貼り付け欄を持たないため，コピーと公式画面への移動を分離しない．共有画面からモノタロウが提供する通常のクイックオーダーフォームへ，注文コード`q0`〜`q9`，数量`p0`〜`p9`，公式の処理指定をPOSTする．1回10商品までのバッチボタンを表示し，Cart2BOM未導入環境でもバスケットへ追加できるようにする．送信済みボタンは無効化し，二重追加を防止する．共有画面の右操作欄では，店舗注文を共有・ファイル出力より上に表示する．
