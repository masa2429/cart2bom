...
クイックオーダー画面は注文コードと数量の10行入力であり，秋月の一括注文形式とは異なる．保存リストから開く場合は，利用者の明示操作後に入力内容をブラウザ内へ一時保存する．新しいクイックオーダー画面では注文コードを短い間隔で順次入力し，全商品の照会を並行して待ってから数量を入力する．11商品以上は10商品ごとのバッチを利用者が選択する．「バスケットに入れる」ボタンは操作せず，利用者が各バッチの内容を確認して実行する．モノタロウ上の保存済みリストには，モノタロウ商品を含むリストだけを表示する．
