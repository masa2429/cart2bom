...
5. 「保存済みリスト」から、再編集、複製、名前変更、削除、各形式の出力を行います。保存済みリストは、現在開いている店舗の商品を含むものだけが表示されます。

保存済みリストには商品画像と明細から算出した合計金額を表示します。価格を取得できない商品がある場合は、合計へ含めず対象商品数を併記します。

秋月では、保存済みリストから「秋月一括注文からバスケットへ自動追加」を押すと、ブランケットオーダーの標準フォームで全商品をまとめて買い物かごへ追加します。Cart2BOMは注文を確定しません。

モノタロウでは、保存済みリストから「モノタロウクイックオーダーからバスケットへ自動追加」を押すと、商品を10件ずつクイックオーダーへ送信します。11商品以上もバスケット画面とクイックオーダー画面を自動で往復して最後まで追加します。購入確定は行わないため、処理後にバスケットの商品、数量、価格、納期を確認してください。
モノタロウがクイックオーダー送信後に`/monotaroMain.py`でバスケットを表示する場合も、商品DOMを確認して正常完了として扱います。

ミスミでは、カート読み取り時に商品を画面上で選択し、最新の単価と出荷日が表示されるまで待ってから保存します。保存済みリストの「ミスミ見積・注文からバスケットへ自動追加」では、「エクセルから一括コピー」へ型番、数量、メーカー名を渡してカートへ追加します。注文・見積の確定には進みません。
