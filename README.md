...
共有画面では、店舗やチェックボックスで商品を絞り込み、平文のコピー、CSV、TSV、JSON、Markdownの保存、店舗別の一括入力ができます。秋月電子通商とミスミは、入力データのコピーと公式画面への移動を1つのボタンで行います。Cart2BOMをインストール済みなら、そのままバスケットへの追加まで自動で進みます。未導入の場合も、コピー済みのデータを公式画面へ手動で貼り付けられます。「Cart2BOMへ取り込む」では共有リストをブラウザ内へ保存できます。
