...
モノタロウでは、保存済みリストから「モノタロウクイックオーダー画面へ入力」を押すと、新しいクイックオーダー画面へ注文コードを短い間隔で順次入力し、商品照会後に数量を入力します。これは秋月の一括注文形式とは異なる、10行フォームへの入力支援です。11商品以上は10商品ごとの入力回を選択し、各回の内容を確認して利用者自身でバスケットへ追加してください。
...
- 注文確定ボタンを操作しません。
- モノタロウのクイックオーダー入力は、明示操作後にブラウザ内へ一時保存し、入力成功時または有効期限切れの確認時に削除します。
