...
Cart2BOMは、通販サイトのカートを保存・共有可能な部品リストへ変換するUserScriptです。秋月電子通商とモノタロウに対応し、CSV、TSV、JSON、Markdownへ出力できます。秋月の商品は一括注文形式にも変換できます。
