...
モノタロウでは、保存済みリストから「モノタロウクイックオーダー画面へ入力」を押すと、新しいクイックオーダー画面へ注文コードと数量を入力します。これは秋月の一括注文形式とは異なる、10行フォームへの入力支援です。11商品以上は10商品ごとの入力回を選択し、各回の内容を確認して利用者自身でバスケットへ追加してください。
