...
表示テーマは「設定」から自動、ライト、ダークを選べます。自動ではOSの配色設定に追従します。GitHub Pagesの共有・インストール画面もOS設定に自動追従します。
...
共有画面では、店舗やチェックボックスで商品を絞り込み、平文のコピー、CSV、TSV、JSON、Markdownの保存、店舗別の一括入力ができます。秋月電子通商は、Cart2BOM未導入でも通販コードと数量を入力済みのブランケットオーダー画面を開けます。ミスミは型番・数量・メーカー名をコピーして公式画面を開き、「エクセルから一括コピー」欄へ一度貼り付けます。モノタロウは10商品ごとに、公式クイックオーダーの `q0/p0` から `q9/p9` へ貼り付ける注文コードと数量を個別にコピーできます。Cart2BOM導入済みの場合、秋月とミスミはバスケットへの追加まで自動で進みます。「Cart2BOMへ取り込む」では共有リストをブラウザ内へ保存できます。
...
- Cart2BOMの共有画面は共有リストを保存せず、アクセス解析用コードや外部APIへの送信も実装していません。商品画像だけは、表示時に各通販サイトの公式画像URLから読み込まれます。なお、通常のWebサイトと同様に、GitHub Pagesの配信基盤にはアクセス時のIPアドレスなどが送られます。
