...
- 「共有URLをコピー」：DiscordやメールでURLを送ります。受け取った人がCart2BOMをインストールしたブラウザでURLを開くと、リスト名、商品数、店舗別件数、合計金額を確認してから取り込めます。
- 「平文をコピー」：商品名、通販コード・型番、メーカー、数量、価格、備考、商品URLを、チャットへそのまま貼れる文章としてコピーします。

共有URLはリストを圧縮してURLの`#cart2bom=`以降へ埋め込むため、共有用サーバーやアカウントは不要です。URLを開いただけではカートへの追加や注文を行いません。同名リストがある場合は、取り込み前に上書きを確認します。

URLにはリストの商品情報が含まれます。URLを知っている人は内容を読み取れるため、公開範囲に注意してください。商品数が多くURLが長くなる場合は、平文またはJSONファイルを利用してください。
...
配布用UserScriptは`dist/cart2bom.user.js`へ生成されます。開発用ソースマップが必要な場合は`npm run build:dev`を使用します。
