...
export class AkizukiAdapter implements StoreAdapter {
  ...
  public isQuickOrderPage(url: URL, _document: Document): boolean {
    return this.matches(url)
      && url.pathname.toLowerCase() === "/catalog/quickorder/blanketorder.aspx";
  }

  /** Posts all items through Akizuki's native blanket-order form. */
  public submitQuickOrder(targetDocument: Document, text: string): number {
    const rows = text.split(/\r?\n/).filter((line) => line.trim()).map((line) => line.split("\t"));
    const sourceForm = targetDocument.querySelector<HTMLFormElement>("form#blanketorder_form");
    if (!sourceForm?.querySelector('textarea[name="regist_goods"]')) {
      throw new Error("秋月のブランケットオーダーフォームを確認できませんでした。");
    }
    ...
    form.hidden = true;
    const orderText = targetDocument.createElement("textarea");
    orderText.name = "regist_goods";
    orderText.value = rows.map(([code = "", quantity = ""]) => `${code}\t${quantity}`).join("\n");
    const submitAction = targetDocument.createElement("input");
    submitAction.type = "hidden";
    submitAction.name = "blanketorder";
    submitAction.value = "買い物かごに入れる";
    form.append(orderText, submitAction);
    targetDocument.body.append(form);
    const nativeSubmit = targetDocument.defaultView?.HTMLFormElement.prototype.submit;
    if (!nativeSubmit) throw new Error("秋月のブランケットオーダーを送信できませんでした。");
